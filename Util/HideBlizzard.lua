...
function CUF:HideBlizzardUnitFrame(type)
    ...
    elseif type == "playerCastBar" then
        -- WotLK: CastingBarFrame is the player cast bar (not PlayerCastingBarFrame)
        if _G.CastingBarFrame then
            HideFrame(_G.CastingBarFrame)
        elseif _G.PlayerCastingBarFrame then
    ...
    elseif type == "boss" then
        -- WotLK: No BossTargetFrameContainer, just individual boss frames
        ...
        for i = 1, MAX_BOSS_FRAMES do
            ...
            if bossFrame then
                HideFrame(bossFrame, true)
            end
        end
    elseif type == "buffFrame" then
        -- WotLK: BuffFrame might not exist or have a different name
        if _G.BuffFrame then
            HideFrame(_G.BuffFrame)
        end
    elseif type == "debuffFrame" then
        -- WotLK: DebuffFrame might not exist
