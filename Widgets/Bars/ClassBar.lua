...
-- WotLK: Only ComboPoints and Runes are supported
local CLASS_POWER_ID = {
    ["ROGUE"] = Enum.PowerType.ComboPoints,
    ["DRUID"] = Enum.PowerType.ComboPoints,
    ["DEATHKNIGHT"] = Enum.PowerType.Runes,
    -- Monk, Paladin, Warlock, Mage, Evoker don't have these in WotLK
}
...
-- WotLK: No required spec checks (no Monk/Mage arcane)
local REQUIRED_SPEC = {}

-- WotLK: No partial resources
...
local function UpdateColors(self)
    ...
    -- WotLK: Only ComboPoints and Runes are supported
    if self.classPowerID == Enum.PowerType.ComboPoints then
    ...
    elseif self.classPowerID == Enum.PowerType.Runes then
        -- WotLK: Use a single rune color since spec system doesn't exist
        -- Try to get frost rune as default, fallback to class colors
        local color = DB.GetColors().runes and DB.GetColors().runes.frostRune
        if not color then
            -- Fallback: use DK class color
            local classColor = RAID_CLASS_COLORS and RAID_CLASS_COLORS["DEATHKNIGHT"]
            if classColor then
                color = { classColor.r, classColor.g, classColor.b, 1 }
            else
                color = { 0.77, 0.12, 0.23, 1 } -- DK red
            end
        end
        ...
        -- Fallback for unknown power types
        for i = 1, self.maxPower do
...
local function TogglePowerEvents(self, enable)
    if enable then
        if self._owner.states.class == "DEATHKNIGHT" then
            ...
            -- WotLK: No Evoker essence or charged combo points
