...
local function OnCellInitialUpdateLayout(_layout)
    ...
    CUF.vars.isRetail = false -- WotLK: Never Retail
    ...
    -- WotLK Fix: Ensure frames are visible after init
    -- RegisterUnitWatch may not trigger properly, so we force a visibility update
    C_Timer.After(0.5, function()
        CUF:Fire("UpdateVisibility")
        -- Force show all enabled unit buttons
        for _, unit in pairs(CUF.constants.UNIT) do
            local layout = CUF.DB.CurrentLayoutTable()
            if layout and layout[unit] and layout[unit].enabled then
                local button = CUF.unitButtons[unit]
