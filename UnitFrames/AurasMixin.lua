...
function AurasMixin:UpdateAurasInternal(event, unit, unitAuraUpdateInfo)
    ...
    -- WotLK: Always do full update since incremental aura info doesn't exist
    -- The unitAuraUpdateInfo parameter is a Retail-only feature
    self:ParseAllAuras(self._ignoreBuffs, self._ignoreDebuffs, unit)
    self:TriggerAuraCallbacks(true, true, true, true, true)
end
